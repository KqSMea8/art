...
<html>
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="/Public/css/richwap.css?v=0.0.3">
    <style type="text/css">
      html {
  ...
  <script type="text/javascript">
    ...
    // var api = window.location.origin + '/V20/' + route + '?token=' + GetRequest().token;
    ...
    $.ajax({
      ...
      success: function(res) {
        if (res.code == 30000) {
          ...
          //var content = res.data.info.article.content.replace(/(\d+)px/g, function(s, t) { // /(\d)+\.?[0-9]+(px)/gi
            //2018.06.11
          var content = res.data.info.article.content.replace(/(\d)+\.?[0-9]+(px)|(\d)+(px)/gi, function(s, t) {
            s = s.replace('px', '');
