...
class UserFollowerLogic extends BaseLogic
{
    ...
    {
        $data = $this->model->where(['user_id'=>$userId, 'is_follow'=>'Y'])->order('follow_time DESC,id DESC')->page($page, $perPageCount)->select();
        if (empty($data)) {
            return [];
        }
        return $data;
    ...
    {
        $data = $this->model->where(['follower'=>$follower, 'is_follow'=>'Y'])->order('follow_time DESC,id DESC')->page($page, $perPageCount)->select();
        if (empty($data)) {
            return [];
        }
        return $data;
    ...
    //我的关注的数量
    public function getFollowTotal($follower)
    {
        $count = $this->model->where(['follower'=>$follower, 'is_follow'=>'Y'])->count();
        return $count;
    }
    ...
    {
        $count = $this->model->where(['user_id'=>$userId, 'is_follow'=>'Y'])->count();
        return $count;
    }
}
