...
if(empty($_POST)){
	...
	// $url="http://test-api.artzhe.com/mp/ArtworkGoods/getArtworkList";
	// $postStr =array(
	// 		'artist'=>0,//暂时写死
	// 		'artworkName'=>'',
	// 		'page'=>1,
	// 		'pagesize'=>10,
	// 	);
	// $allow_status=array(2,4);//没有关联的原作状态和都没关联的状态 (1,2,3,4)
	// $request_data = goods_relationship_art($url,$postStr,$allow_status); //url 请求的接口路径 postStr加密前参数 数组 
	// $smarty->assign('artWorkList',$request_data);
	...
	if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
		...
		if ($_REQUEST['act'] == 'set_attribute_table' || $_REQUEST['act'] == 'goods_attribute_query') {
			...
			if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update'){
					...
					$is_alone_sale = isset($_POST['is_alone_sale']) && !empty($_POST['is_alone_sale']) ? 1 : 1;
					...
					$catgory_id = empty($_POST['cat_id']) ? 1488 : intval($_POST['cat_id']);//商品类别  1488--原作
					...
					if ($is_insert) {
						...
						=> $goods_weight, 'is_on_sale' => $is_on_sale, 'user_id' => $_SESSION['seller_id'], 'handle_time' => gmtime(), 'old_record' => 0);
					...
					if ($is_insert) {
						$url="https://test-api.artzhe.com/mp/ArtworkGoods/relateGoods";
				                    	$postStr=array(
				                    			'goodsId'=>$goods_id,
						...
						if($api && $goods_id>0){
							...
							if($goods_id>0 && isset($_POST['goods_img_gallery']) && !empty($_POST['goods_img_gallery'])){ //新增时候操作gallery表 此为API操作
								...
								if(is_array($gallery_obj)){
									...
									foreach($gallery_obj as $k=>$v){
										if(is_array($v)){
											$sql_new ='INSERT INTO' . $ecs->table('goods_gallery') . ' (goods_id,img_url,img_desc,thumb_url,img_original,single_id,external_url,front_cover,dis_id)' . " VALUES ($goods_id,'{$v['img_original']}',{$v['img_desc']},'{$v['img_original']}','{$v['img_original']}', 0,'',0,0)";
										}
										$db->query($sql_new);
									}
								}
					...
					if(isset($_POST['is_mount']) && isset($_POST['is_collect'])){
						...
						$attr_catory_id = $db->getOne('SELECT attr_id FROM ' . $ecs->table('attribute') . (' WHERE attr_name = \'' . '类型' . '\''));
						$attr_color_id = $db->getOne('SELECT attr_id FROM ' . $ecs->table('attribute') . (' WHERE attr_name = \'' . '色调' . '\''));
						$attr_shape_id = $db->getOne('SELECT attr_id FROM ' . $ecs->table('attribute') . (' WHERE attr_name = \'' . '形状' . '\''));
						$attr_length_id = $db->getOne('SELECT attr_id FROM ' . $ecs->table('attribute') . (' WHERE attr_name = \'' . '长度' . '\''));
						$attr_width_id = $db->getOne('SELECT attr_id FROM ' . $ecs->table('attribute') . (' WHERE attr_name = \'' . '宽度' . '\''));
						$attr_diameter_id = $db->getOne('SELECT attr_id FROM ' . $ecs->table('attribute') . (' WHERE attr_name = \'' . '直径' . '\''));
						$attr_date_id = $db->getOne('SELECT attr_id FROM ' . $ecs->table('attribute') . (' WHERE attr_name = \'' . '创作年份' . '\''));
						$attr_subjects_id = $db->getOne('SELECT attr_id FROM ' . $ecs->table('attribute') . (' WHERE attr_name = \'' . '题材' . '\''));
						$attr_styles_id = $db->getOne('SELECT attr_id FROM ' . $ecs->table('attribute') . (' WHERE attr_name = \'' . '风格' . '\''));

						$_POST['attr_id_list']=array($attr_mount_id,$attr_collect_id,$attr_catory_id,$attr_color_id,$attr_shape_id,$attr_length_id,$attr_width_id,$attr_diameter_id,$attr_date_id,$attr_subjects_id,$attr_styles_id);
						if(empty($_POST['is_mount']) && empty($_POST['is_collect']) && empty($_POST['goods_artwork_catory']) && empty($_POST['goods_artwork_color'])
							&& empty($_POST['goods_artwork_shape']) && empty($_POST['goods_artwork_date']) && empty($_POST['goods_artwork_subjects'])&& empty($_POST['goods_artwork_styles'])){
							exit(json_encode(array(
								'error'=>1,
								'msg'=>'艺术品属性缺少',
							)));
						}else{
							if(trim($_POST['goods_artwork_shape']) == '方形' && empty($_POST['goods_artwork_length']) && empty($_POST['goods_artwork_width'])){
								exit(json_encode(array(
									'error'=>1,
									'msg'=>'艺术品属性缺少',
								)));
							}elseif(trim($_POST['goods_artwork_shape']) == '圆形' && empty($_POST['goods_artwork_diameter'])){
								exit(json_encode(array(
									'error'=>1,
									'msg'=>'艺术品属性缺少',
								)));
						...
						$is_mount_value = $_POST['is_mount']?'是':'否';
						$is_collect_value = $_POST['is_collect']?'是':'否';
						$catory_value = $_POST['goods_artwork_catory']?$_POST['goods_artwork_catory']:'';
						$color_value = $_POST['goods_artwork_color']?$_POST['goods_artwork_color']:'';
						$shape_value = $_POST['goods_artwork_shape']?$_POST['goods_artwork_shape']:'';
						$length_value = $_POST['goods_artwork_length']?floatval($_POST['goods_artwork_length']):'';
						$width_value = $_POST['goods_artwork_width']?floatval($_POST['goods_artwork_width']):'';
						$diameter_value = $_POST['goods_artwork_diameter']?floatval($_POST['goods_artwork_diameter']):'';
						$date_value = $_POST['goods_artwork_date']?$_POST['goods_artwork_date']:'';
						$subjects_value = $_POST['goods_artwork_subjects']?$_POST['goods_artwork_subjects']:'';
						$styles_value = $_POST['goods_artwork_styles']?$_POST['goods_artwork_styles']:'';

						$_POST['attr_value_list']=array($is_mount_value,$is_collect_value,$catory_value,$color_value,$shape_value,$length_value,$width_value,$diameter_value,$date_value,$subjects_value,$styles_value);
						$_POST['attr_price_list']=[];
					}
