...
if ($_REQUEST['act'] == 'logout') {
    ...
    $token = $_COOKIE['web_token'];
    $baseUrl = 'http://test-passport.artzhe.com/passport/user';
    $getUserUrl = $baseUrl."/logout";
    $postStr = des_encode(json_encode(array('token'=>$token,'user_ip'=>$_SERVER['REMOTE_ADDR'])));
...
//	header('Location:'.'http://test-mp.artzhe.com/');
...
// 在这里修改登录的方式 'http://test-mall.artzhe.com/seller/privilege.php?act=signin&az_from=mpartzhe'
else if ($_REQUEST['act'] == 'signin') {

    if ($_REQUEST['az_from'] == 'mpartzhe'){//创作中心登录
        $token = $_COOKIE['web_token'];
        $baseUrl = 'http://test-passport.artzhe.com/passport/user';
        ...
        $row = false;
        if ($data){
            $sql = 'SELECT user_id, ru_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt' . ' FROM ' . $ecs->table('admin_user') . ' WHERE ru_id = \'' . $data['data']['id'] . '\' ';
            $row = $db->getRow($sql);
        }
    }else{

//        header('Location:'.'http://test-mp.artzhe.com/');exit();
