...
$param=isset($_POST['param'])?json_decode(des_decode($_POST['param']),true):[];
if(!empty($param) && $param['is_api'] ){
	$_POST=$param;
	$_POST = addslashes_deep($_POST);
	$api=isset($_POST['is_api'])?$_POST['is_api']:0;
}
$shield=1;
if(isset($api) && !empty($api)){$shield=0;}
...
if ($_REQUEST['act'] != 'login' && $_REQUEST['act'] != 'signin' && $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order' && $shield) {
...
if (!in_array($_REQUEST['act'], $is_act) && $shield) {
...
if(!function_exists('curl_request')){
 ...
 if(!function_exists('goods_relationship_art')){
 	function goods_relationship_art($url,$postStr,$allow_status,$goods_id=0){
	$post_data=array('param'=>des_encode(json_encode($postStr)));
	$return_data = curl_request($url,$post_data);
	$return_data= json_decode($return_data,true);
	
	if(isset($return_data) && !empty($return_data) && is_array($return_data['data']['info']['list']) && $return_data['data']['status'] ==1000){
		foreach($return_data['data']['info']['list'] as $k=>$v){
			    $goods=[];
				$retName['info'][$k]=array(
					'id'=>$v['id'],
					'name'=>$v['name'],
					'img'=>$v['cover'],
				);
				if(isset($v['goods_ids']) && !empty($v['goods_ids'])){
					$goods =explode(',',$v['goods_ids']);
				}
				$retName['info'][$k]['status']=in_array($goods_id,$goods)?1:0;
		}
		$retName['page'] =intval($return_data['data']['info']['page']);
		$retName['maxpage'] = intval($return_data['data']['info']['maxpage']);

		return is_array($retName)?$retName:[];
	}else{
		return [];
...
if(isset($_GET['get_art_list']) &&! empty($_GET['get_art_list'])){
	if(!function_exists('show_art_list')){
		$page=isset($_GET['page'])?$_GET['page']:1;
		$goods_id=isset($_GET['goods_id'])?$_GET['goods_id']:0;
		function show_art_list($page=1,$user_id=0,$goods_id=0){
			$url="http://test-api.artzhe.com/mp/ArtworkGoods/getArtworkList";
			$postStr =array(
					'artist'=>empty($goods_id)?$user_id:0,//暂时写死112540$adminru['ru_id'] 
					'artworkName' =>'',
				    'goods_id'=>$goods_id,
					'page'=>$page,
					'pagesize'=>5,
		...
		$data = goods_relationship_art($url,$postStr,$allow_status,$goods_id);
			if(empty($data['info'] )){
					exit(json_encode(array('error'=>1,'data'=>[],'msg'=>'请先创建艺术品之后,再使用商品添加功能')));
			}
			exit(json_encode(array('error'=>0,'data'=>$data,'msg'=>'获取艺术品列表成功')));
		}
	show_art_list($page,$adminru['ru_id'],$goods_id);
	}
}
