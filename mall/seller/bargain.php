...
if ($_REQUEST['act'] == 'list') {
	...
	if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
		$smarty->assign('primary_cat', $_LANG['03_promotion']);
		$smarty->assign('menu_select', array('action' => '03_promotion', 'current' => '19_bargain'));
		...
		if (0 < $id) {
			...
			$sql = 'SELECT count(goods_id) as num  FROM ' . $GLOBALS['ecs']->table('bargain_goods') . ' WHERE goods_id = \'' . $goods['goods_id'] . '\' and status = \'1\' ';
			...
			$links = array(
				array('href' => 'bargain.php?act=add', 'text' => '继续添加砍价商品'),
				array('href' => 'bargain.php?act=list', 'text' => '返回拼团砍价列表')
				);
