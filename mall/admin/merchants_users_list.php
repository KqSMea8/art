...
if ($_REQUEST['act'] == 'add_shop' || $_REQUEST['act'] == 'edit_shop' || $_REQUEST['act'] == 'copy_shop') {
    ...
    if ($_REQUEST['act'] == 'insert_shop' || $_REQUEST['act'] == 'update_shop') {
        ...
        if ($merchants_audit != 0) {
            $_CFG = load_config();
            $url = $_CFG['artzhe_url'] . 'ArtworkGoods/sendMessage';
            $content = '';
            if ($merchants_audit == 1) {//入驻成功
                $content = $_CFG['artzhe_merchant_success'];
            } else {//入驻失败
                $content = $_CFG['artzhe_merchant_failure'];
            }
            include_once ROOT_PATH . 'vendor/crypt.php';
            /*include_once ROOT_PATH . 'includes/cls_json.php';
            $json = new JSON();*/
            $postStr = des_encode(json_encode(['userId' => $user_id, 'content' => $content]));
        ...
        $merchants = $db->getRow($sql);
        $sql = 'select user_name, email, user_id ,user_picture from ' . $ecs->table('users') . (' where user_id = \'' . $user_id . '\'');
        $user = $db->getRow($sql);
        $user_img = empty($user['user_picture']) ? '' : $user['user_picture'];
        ...
        //TODO 设置店铺用户信息
        if (0 < $adminId) {
            $sql = 'update ' . $ecs->table('admin_user') . (' set user_name = \'' . $user_name . '\', email = \'' . $email . '\', ') . ('nav_list = \'' . $row['nav_list'] . '\', action_list = \'' . $action_list . '\', admin_user_img = \'' . $user_img . '\' where ru_id = \'' . $user_id . '\' AND parent_id =0');
        } else {
            $sql = 'INSERT INTO ' . $ecs->table('admin_user') . ' (user_name, email, password, ec_salt, nav_list, action_list, ru_id,admin_user_img) ' . ('VALUES (\'' . $user_name . '\', \'') . $email . ('\', \'' . $password . '\', \'' . $ec_salt . '\', \'' . $row['nav_list'] . '\', \'' . $action_list . '\', \'' . $user_id . '\', \'' . $user_img . '\')');
        }
		...
		//设置默认店铺logo   用户头像
		$user_picture = $db->getOne(' SELECT user_picture FROM' . $ecs->table('users') . ' WHERE user_id=\'' . $adminru['ru_id'] . '\'');
		$seller_shop_info['logo_thumb'] = empty($seller_shop_info['logo_thumb'])?$user_picture:$seller_shop_info['logo_thumb'];
		...
		//设置商家店铺的默认信息
		$seller_shop_info['shop_name'] = empty($seller_shop_info['shop_name'])?$shop_information['rz_shopName']:$seller_shop_info['shop_name'];
		$seller_shop_info['country'] = empty($seller_shop_info['country'])?1:$seller_shop_info['country'];
		$seller_shop_info['province'] = empty($seller_shop_info['province'])?6:$seller_shop_info['province'];
		$seller_shop_info['city'] = empty($seller_shop_info['city'])?77:$seller_shop_info['city'];
		$seller_shop_info['district'] = empty($seller_shop_info['district'])?707:$seller_shop_info['district'];
		$seller_shop_info['shipping_id'] = empty($seller_shop_info['shipping_id'])?16:$seller_shop_info['shipping_id'];
		...
		if ($shop_info['logo_thumb']) {
			...
			//设置默认店铺logo   用户头像
			$user_picture = $db->getOne(' SELECT user_picture FROM' . $ecs->table('users') . ' WHERE user_id=\'' . $adminru['ru_id'] . '\'');
			$shop_info['logo_thumb'] = $user_picture;
		}
