...
if(empty($_POST)){
	...
	$goods_data = $db->query('select g.goods_name,g.shop_price,g.goods_number,g.is_shipping,g.shipping_fee,g.desc_mobile,g.cat_id,g.goods_thumb,a.attr_value,a.attr_id from '.$ecs->table('goods').' as g left join '.$ecs->table('goods_attr').' as a on a.goods_id =g.goods_id where g.goods_id ='.$goods_id);
	...
	while ($row = $db->fetchRow($goods_data )) {
		...
		$cat_data = $db->query('select g.attr_value,g.attr_id,a.attr_name from '.$ecs->table('goods_attr').' as g left join'.$ecs->table('attribute').'  as a on g.attr_id=a.attr_id '.'where g.goods_id='.$goods_id.' AND a.attr_name IN ("版画规格","画布类型","创作年份")');

		while ($row =$db->fetchRow(	$cat_data)) {
				$cat_info[]=array(
						'attr_id'=>$row['attr_id'],
	...
	while ($row = $db->fetchRow($cat_info)) {
		if($goods_info['cat_id'] == $row['cat_id']){
			$check_cat_id=1;
		}
		$category_info[]=array(
				'cat_name'=>$row['cat_name'],
				'cat_id'=>$row['cat_id'],
				'check_cat_id'=>isset($check_cat_id)?$check_cat_id:0,
			);
			unset($check_cat_id);
	}
	...
	$smarty->assign('goods_number',$goods_info['goods_number']);
	...
	if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
		...
		if ($_REQUEST['act'] == 'set_attribute_table' || $_REQUEST['act'] == 'goods_attribute_query') {
			...
			if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update'){
					...
					$shop_price = empty($_REQUEST['shop_price']) ? '' : intval(trim($_REQUEST['shop_price']));
					...
					$shipping_fee =isset($_REQUEST['shipping_fee']) ? intval($_REQUEST['shipping_fee'] ):'' ;
					$cat_id =isset($_REQUEST['cat_id']) ? intval($_REQUEST['cat_id'] ):1483 ;
...
exit();
					...
					$original_img = empty($_REQUEST['original_img']) ? '' : trim($_REQUEST['original_img']);
					$goods_img = empty($_REQUEST['original_img']) ? '' : trim($_REQUEST['original_img']);
					$goods_thumb = empty($_REQUEST['original_img']) ? '' : trim($_REQUEST['original_img']);
