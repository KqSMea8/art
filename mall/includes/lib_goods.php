...
function get_goods_info($goods_id, $warehouse_id = 0, $area_id = 0, $select = array())
{
	...
	$sql = 'SELECT ' . $select . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('category') . ' AS c ON g.cat_id = c.cat_id ' . $leftJoin . ' LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . ('ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ') . ('WHERE g.goods_id = \'' . $goods_id . '\' AND g.is_delete = 0 ') . 'GROUP BY g.goods_id';
