...
class Integrate
{
	...
	// 添加艺术者商城的登录方法 设置为登录状态即可
	// 同步用户信息和添加新用户均可以在此操作
    public function az_login($data, $remember = NULL)
    {
        $user_id = $data['id'];
        $sql = 'SELECT ' . 'user_name' . ' FROM ' . $this->table($this->user_table) . ' WHERE ' . $this->field_id . '=\'' . $user_id . '\'';
        $result = $this->db->getOne($sql);
        $username = $result;

        if(!$result){
            $username =$this->az_insert_user($data);
        }else{
            $username = $this->az_update_user($data);
        }
		...
		if($data['wechat_openId']){
            $_SESSION['openid'] = $data['wechat_openId'];
		}
        return true;
	}

	// 更新用户信息
	private function az_update_user($data){
	    $user_id = $data['id'];
        $updateSql = ' ';
        $updateSql = $updateSql.'nick_name = '.'\''.$data['name'].'\'';
        $updateSql = $updateSql.' , user_picture = '.'\''.$data['face'].'\'';
        $updateSql = $updateSql.' , mobile_phone = '.'\''.$data['mobile'].'\'';
        $sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . ' SET '.$updateSql .' where '. $this->field_id . '=\'' . $user_id . '\'';
        $this->db->query($sql);

        Log::write('Update Sql:',$sql, Log::DEBUG);

        // 用户名是唯一的索引
        $sqlSelect = 'SELECT ' . 'user_name' . ' FROM ' . $this->table($this->user_table) . ' WHERE ' . $this->field_id . '=\'' . $user_id . '\'';
        $result = $this->db->getOne($sqlSelect);
        $username = $result;
    ...
    // 新插入一个用户
    private function az_insert_user($data){
        $row = $data;
        $mobile = $row['mobile'];
        $user_id = $row['id'];
        ...
        Log::write('INSERT Sql:',$s1, Log::DEBUG);
     ...
     * @param string $md5password 加密后的密码
     * @return bool bool 返回值
     */
