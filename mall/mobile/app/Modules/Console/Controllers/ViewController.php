		...
		if (IS_POST) {
			...
			if (empty($article_msg)) {
				...
				foreach ($article_msg as $key => $value) {
					...
					$length = strlen($value['link']);

					if (7 < $length) {
						$article_msg[$key]['url'] = $value['link'];
					}
					else {
	...
	public function actionProduct()
	{
		if (IS_POST) {
			...
			if (!empty($goods_id)) {
				...
				/*$product_cache = md5('cat0' . $cat_id . $number.$user_id);//md5('cat0' . $cat_id . $number)
				...
				if ($product === false || empty($product)) {
					...
					//$type=>''
					$product = category_get_goods($children, '', $brand, $user_id, '', $warehouse_id, $area_id, $number);
		...
		if (IS_POST) {
			...
			if ($id) {
				$view = dao('touch_page_view')->field('type, title, thumb_pic, data, default')->where(array('id' => $id))->find();
					if( $GLOBALS['MALL_CLIENT'] =="APP"){//判断是否为APP端  获取轮播图地址
						if(!empty($view['data']) && is_array(json_decode($view['data'],true))){
							foreach( json_decode($view['data'],true) as $k=>$v){
								if($v['module'] == 'slide'){
									$res=$v;
								}
							}
							...
							$this->response(array('error'=>0,'data'=>[]));
						}	
					}
			}
			else if ($default < 2) {
