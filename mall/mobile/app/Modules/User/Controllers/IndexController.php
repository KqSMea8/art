...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	public function actionIndex()
	{
		...
		$this->assign('drp',  0);//个人中心的微店
		$this->assign('team', 0);//
		$this->assign('bargain', 0);//
		$this->assign('team', 0);//
		$this->assign('comment', 0);//评价
		$this->assign('crowd', 0);//微筹显示 0表示不显示
		$this->assign('is_show_mymoneny', 0);//我的钱包
		$share = unserialize($GLOBALS['_CFG']['affiliate']);

		// if ($share['on'] == 1) {
		$this->assign('share',0);//不现实分享
		//}
	...
	public function actionchecklogin()
	{
		...
		if (!$_SESSION['user_id']) {
			...
			if ($appToken){
                $this->redirect('user/login/index', array('az_token'=>$appToken,'back_act' => urlencode($back_act)));
            }else{
	...
	public function actionCommentList()
	{
		...
		$record_count = get_user_order_comment_list($_SESSION['user_id'], 1, $sign,$order_id);
		$pager = get_pager('user.php', array('act' => $action), $record_count, $page, $size);
		$comment_list = get_user_order_comment_list($_SESSION['user_id'], 0, $sign, $order_id, $size, $pager['start']);
	...
	public function actionAddComment()
	{
		if (IS_POST) {
			...
			show_message('商品评论成功', '返回上一页', url('user/index/comment_list',['order_id'=>$order_id]), 'success');
		}
