...
abstract class FrontendController extends FoundationController
{
	...
	private function checkUA()
    {
        ...
        if(strpos($user_agent, 'Artzhe') !== false){// APP环境
            $GLOBALS['MALL_CLIENT'] = 'APP';
        }elseif (strpos($user_agent, 'MicroMessenger') !== false){ // 微信环境
            $GLOBALS['MALL_CLIENT'] = 'WECHAT';
		}else{
            $GLOBALS['MALL_CLIENT'] = 'H5';
		}
    }
	...
	private function appLogin(){
        ...
        Log::write('data:'.json_encode($appToken),Log::DEBUG);
        Log::write('data:'.json_encode($user_id),Log::DEBUG);
        Log::write('data:'.json_encode($GLOBALS['MALL_CLIENT']),Log::DEBUG);

        if ($GLOBALS['MALL_CLIENT'] == 'APP' && $user_id < 0 && $appToken) {
            $baseUrl = 'https://test-passport.artzhe.com/passport/user';
            $getUserUrl = $baseUrl."/getinfo";
            $postStr = des_encode(json_encode(array('token'=>$appToken,'user_ip'=>$_SERVER['REMOTE_ADDR'])));
            $data = $this->requireArtzhe($postStr, $getUserUrl);
     ...
     */
    private function requireArtzhe($paramStr, $url){
