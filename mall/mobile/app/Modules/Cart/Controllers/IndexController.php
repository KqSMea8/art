...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	public function actionIndex()
	{
        ...
        Log::write('data:'.json_encode($_SESSION),Log::DEBUG);
		...
		$cart_show = array();
		$heart_all=1;//购物车选中的商品 用户是否都关注了  1--是  0--否
			...
			foreach ($cart_goods['goods_list'] as $k => $list) {
				if ($list['goods_list']) {
					...
					foreach ($list['goods_list'] as $key => $val) {
						...
						if ($val['is_checked'] == 0) {//未选中
							$val['goods_number'] = 0;
						}else{//选中
							if($heart_all == 1){
								$sql = 'SELECT count(rec_id) as a FROM {pre}collect_goods WHERE user_id=' . $_SESSION['user_id'] . ' AND goods_id=' . $val['goods_id'];
		...
		if (C('shop.wap_category') == '1'  && IS_POST) {//区分点击购物次行为和分类信息的行为
			...
			$this->assign('is_app',$GLOBALS['MALL_CLIENT']);
			$this->assign('page_title', '购物车');
			$this->assign('heart_all', $heart_all);//选中商品是否都关注
		}
	...
	{
		$where = ' g.cat_id != 1488 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND ' . 'g.is_delete = 0 AND g.review_status > 2 ';
	...
	public function actionCartLabelCount()
	{
		...
		if ($rec_id) {
			$heart_all=1;//购物车选中的商品 用户是否都关注了  1--是  0--否
			$sql = 'SELECT goods_id FROM {pre}cart WHERE user_id =' . $_SESSION['user_id'] . ' AND is_checked=1 ';
			$checked_goods = $this->db->getAll($sql);
			foreach ($checked_goods as $key) {
				if ($heart_all == 1) {
					$sql = 'SELECT count(rec_id) as a FROM {pre}collect_goods WHERE user_id=' . $_SESSION['user_id'] . ' AND goods_id=' . $key['goods_id'];
					$info = $this->db->getOne($sql);
					if ($info < 1) {//选中商品未关注
						$heart_all = 0;
						break;
	...
	public function actionAddToCart()
	{
        ...
        if ($GLOBALS['MALL_CLIENT'] == 'APP' && $user_id < 0) {
            $result['error'] = 400;
            $result['url'] = url('index.php?m=user&c=login');
            exit(json_encode($result));
        }
	...
	public function actionHeart()
	{
		if (0 < $_SESSION['user_id']) {
			...
			if(empty($id)){
				exit(json_encode(array('msg' => '至少选中一个商品', 'error' => 0)));
			}
		...
		exit(json_encode(array('msg' => L('yet_login'), 'error' => -1)));
	}
