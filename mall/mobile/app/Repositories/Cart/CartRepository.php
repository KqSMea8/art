...
class CartRepository implements \App\Contracts\Repositories\Cart\CartRepositoryInterface
{
	...
	public function getGoodsInCartByUser($id)
	{
		$cart = \App\Models\Cart::select('cart.*')->with(array('goods' => function($query) {
			$query->select('goods_id', 'goods_name', 'goods_thumb', 'freight', 'tid', 'goods_weight', 'shipping_fee');
	...
	public function computeDiscountCheck($order_products)
	{
		...
		$favourable_list = \App\Models\FavourableActivity::where('start_time', '<=', $now)->where('end_time', '>=', $now)->whereraw('CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'')->wherein('act_type', array(FAT_DISCOUNT, FAT_PRICE))->get()->toArray();
