...
function get_goods_one_attr_price($goods, $warehouse_id = 0, $area_id = 0, $promote_price = 0, $goods_product = array())
{
	...
	if ($GLOBALS['_CFG']['add_shop_price'] == 0 && $goods['model_attr'] == 0) {
		...
		if ($goods_product && 0 < $goods_product['product_price']) {
			...
			$spec_price = 0;
			$properties = get_goods_properties($goods['goods_id'], $warehouse_id, $area_id, '', 0, $goods['model_attr'], 0);
			...
			if (!empty($arr['attr_id'])) {
				...
				if ($products) {
					...
					$other = array('product_table' => $products['product_table'], 'product_id' => $products['product_id'], 'product_price' => $products['product_price'], 'product_promote_price' => $products['product_promote_price']);
