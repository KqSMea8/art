...
function get_user_orders($user_id, $num = 10, $page = 1, $status = 0)
{
	...
	$select = ' (SELECT count(*) FROM ' . $GLOBALS['ecs']->table('comment') . (' AS c WHERE c.comment_type = 0 AND c.order_id = oi.order_id AND c.parent_id = 0 AND c.user_id = \'' . $user_id . '\') AS sign1, ') . '(SELECT count(*) FROM ' . $GLOBALS['ecs']->table('comment_img') . ' AS ci, ' . $GLOBALS['ecs']->table('comment') . ' AS c' . (' WHERE c.comment_type = 0 AND c.order_id = oi.order_id AND c.parent_id = 0 AND c.user_id = \'' . $user_id . '\' AND ci.comment_id = c.comment_id )  AS sign2, ');
	...
	foreach ($res as $key => $row) {
		...
		if ($row['sign1'] == 0 || $row['sign1'] < count($row['order_goods'])) {
			$row['sign'] = 0;
		}
		else {
		...
		if ($row['user_order'] == OS_SPLITED && $row['user_shipping'] == SS_RECEIVED && $row['user_pay'] == PS_PAYED) {
			...
			if (0 < $row['sign']) {
				...
				$sign = '';
				$sign_url = url('user/index/comment_list',['order_id'=>$row['order_id']]);
