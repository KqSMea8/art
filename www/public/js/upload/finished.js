var vmAuth = new Vue({
  ...
  data: {
   ...
   created:function(){
     ...
     if(st){
         artzheAgent.call('Artwork/getArtworkDesc',{'artId':that.artID},function(response){ 
           if(response.data.status == 1000 && response.code == 30000){
                var arr1 = [],arr2 = [];
                for(var i = 0;i < response.data.info.panorama_ids.length;i++){
                  arr1[i] = {url:response.data.info.panorama_ids[i]};
                }
                for(var i = 0;i < response.data.info.topography_ids.length;i++){
                  arr2[i] = {url:response.data.info.topography_ids[i]};
                }
