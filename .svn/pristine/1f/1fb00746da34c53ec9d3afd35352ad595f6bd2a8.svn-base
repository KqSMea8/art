...
if(empty($_POST)){
	...
	// $user_id = $db->getOne('select user_id from' . $ecs->table('goods') . ' WHERE (goods_id ="$goods_id" \'\')');
	// if($ru_id != $user_id || empty($goods_id)){
	...
	$goods_data = $db->query('select g.goods_name,g.shop_price,g.is_shipping,g.shipping_fee,g.desc_mobile,g.goods_thumb,a.attr_value,a.attr_id from '.$ecs->table('goods').' as g left join '.$ecs->table('goods_attr').' as a on a.goods_id =g.goods_id where g.goods_id ='.$goods_id);
	while ($row = $db->fetchRow($goods_data )) {
					$goods_info['goods_name'] = $row['goods_name'];
					$goods_info['shop_price'] = $row['shop_price'];
					$goods_info['is_shipping'] = $row['is_shipping'];
					$goods_info['shipping_fee'] = $row['shipping_fee'];
					$goods_info['desc_mobile'] = $row['desc_mobile'];
					$goods_info['goods_thumb'] = $row['goods_thumb'];
					if($attr_id_1 ==$row['attr_id']){
					   $attr_value_1=$row['attr_value'];
					}elseif($attr_id_2 ==$row['attr_id']){
						$attr_value_2=$row['attr_value'];
					}
		}
	if (!preg_match('/(http:\/\/)|(https:\/\/)/i', $goods_info['goods_thumb'])) {
	    $goods_info['goods_thumb']='http://'.$_SERVER['HTTP_HOST'].'/'.$goods_info['goods_thumb'];

	}
	$smarty->assign('goods_name',$goods_info['goods_name']);
	$smarty->assign('shop_price',$goods_info['shop_price']);
	$smarty->assign('is_shipping',$goods_info['is_shipping']);
	$smarty->assign('shipping_fee',$goods_info['shipping_fee']);
	$smarty->assign('desc_mobile',$goods_info['desc_mobile']);
	$smarty->assign('goods_thumb',$goods_info['goods_thumb']);
	...
	$smarty->assign('attr_value_1',$attr_value_1);
	$smarty->assign('attr_value_2',$attr_value_2);
	$smarty->assign('img_list',$img_list);

//	$url="https://test-api.artzhe.com/mp/ArtworkGoods/getArtworkList";
//	$postStr =array(
//			'artist'=>$ru_id,//暂时写死
//			'artworkName'=>'',
//			'page'=>1,
...
//	$request_data = goods_relationship_art($url,$postStr,$allow_status,$goods_id); //url 请求的接口路径 postStr加密前参数 数组
	...
	if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
		...
		if ($_REQUEST['act'] == 'set_attribute_table' || $_REQUEST['act'] == 'goods_attribute_query') {
			...
			if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update'){
					...
					$shop_price = empty($_REQUEST['shop_price']) ? '' : floatval(trim($_REQUEST['shop_price']));
					$original_img = empty($_REQUEST['original_img']) ? '' : trim($_REQUEST['original_img']);
					$is_shipping = isset($_REQUEST['is_shipping']) ? intval($_REQUEST['is_shipping']) : '';
					$shipping_fee =isset($_REQUEST['shipping_fee']) ? floatval($_REQUEST['shipping_fee'] ):'' ;
					$attr_id_list = empty($_REQUEST['attr_id_list']) ? '' : $_REQUEST['attr_id_list'];
					$attr_value_list = empty($_REQUEST['attr_value_list']) ? '' : $_REQUEST['attr_value_list'];
					$desc_mobile = empty($_REQUEST['desc_mobile']) ? '' : trim($_REQUEST['desc_mobile']);
					if (isset($_POST['attr_id_list']) && isset($_POST['attr_value_list']) || empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])) {
						...
						while ($row = $db->fetchRow($res)) {
							$goods_attr_list[$row['attr_id']][$row['attr_value']] = array('sign' => 'older', 'goods_attr_id' => $row['goods_attr_id']);
						}
						...
						foreach ($goods_attr_list as $attr_id => $attr_value_list) {
							foreach ($attr_value_list as $attr_value => $info) {
								...
								}else if ($info['sign'] == 'older') {
									continue;
								}
								else if ($info['sign'] == 'delete'){
									if ($model_attr == 1) {
					...
					if($shop_price){
						$sql .= 'shop_price = \'' . $shop_price . '\', ';
					}
					if($original_img){
						$sql .= 'original_img = \'' . $original_img . '\',goods_thumb = \'' . $original_img . '\',goods_img = \'' . $original_img . '\',';
					}

					$sql .= 'is_shipping = \'' . $is_shipping . '\', ';
						$sql .=' review_status =1 ,';
					if($is_shipping ==1){
						$sql .= 'shipping_fee =  \''.'0.00'.'\'';
					}else{
						$sql .= 'shipping_fee = \'' . $shipping_fee . '\' ';
					}
					
					if($desc_mobile){
						$sql .= ',desc_mobile = \'' . $desc_mobile . '\' ';
					}
...
exit();
					...
					if(isset($_POST['is_mount']) && isset($_POST['is_collect'])){
							...
							$_POST['attr_value_list']=array($_POST['is_mount']?'是':'否',$_POST['is_collect']?'是':'否');
