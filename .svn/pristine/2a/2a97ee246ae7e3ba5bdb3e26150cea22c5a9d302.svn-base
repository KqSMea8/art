...
var vmApp = new Vue({
  ...
  // initial data
  data: {
    // articleDetail: JSON.stringify({'type': 'articleDetail','id': 150}),
    // artworkUpdate: JSON.stringify({'type': 'artworkUpdate','id': 150}),
    // artworkComment: JSON.stringify({'type': 'artworkComment','id': 150}),
    // galleryDetail: JSON.stringify({'type': 'galleryDetail','id': 100028}),
    // artworkDetail: JSON.stringify({'type': 'artworkDetail','id': 150}),
    // articleComment: JSON.stringify({'type': 'articleComment','id': 150}),
    isLoading: true,
    likeClick: false,
    followClick: false,
    zanClick: false,
  ...
  methods: {
    ...
    checklikeandnews: function(ids){ //更新喜欢，留言数量
      ...
      var route = 'MobileGetH5/ArtCircleStatisticalData';
      var api = window.location.origin + '/V43/' + route;
      $.ajax({
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        ...
        success: function(res){
          if (res.code == 30000) {
            var alint2 = document.querySelectorAll('.abc');
            for (var adx1 = 0; adx1<alint2.length; adx1++){
              if(res.data.info[alint2[adx1].getAttribute('data-artzhe-id')]){
                document.querySelectorAll('.smalltrp')[adx1].children[0].innerHTML=res.data.info[alint2[adx1].getAttribute('data-artzhe-id')].like_total;
                document.querySelectorAll('.smalltrp')[adx1].children[1].innerHTML=res.data.info[alint2[adx1].getAttribute('data-artzhe-id')].comment_total;
              }
            }
          }
        }
