...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	public function actionIndex()
	{
		...
		uaredirect(__PC__ . '/');
		if (IS_POST) {
			if($GLOBALS['MALL_CLIENT'] =="APP" ){
				$parent_category_name='艺术品';
				$search_category_name_yuan="原作";
				$search_category_name_ban="版画";//目前为版画和原作
		        $parent_cat_id = dao('category')->where(array('cat_name'=>$parent_category_name,'parent_id'=>0))->getField('cat_id');
		        if(isset(  $parent_cat_id ) &&  $parent_cat_id >0){
		        	$data_category =dao('category')->where(array('cat_name'=>array('in',array($search_category_name_yuan,$search_category_name_ban)),'parent_id'=> $parent_cat_id ))->field('cat_id,cat_name')->select();
		        	if(!empty($data_category)){
		        		$this->response(array('error'=>0, 'data'=>$data_category? $data_category : ''));
		        	}
		        }
			}
		...
		}elseif(IS_GET && $GLOBALS['MALL_CLIENT'] =="APP"){
			$this->display('index.app');exit();
		}
		...
		$share_title = !empty($pages['title']) ? $pages['title'] : C('shop.shop_name');
		...
		$share_data = array('title' => $share_title, 'desc' => $share_desc, 'link' => '', 'img' => $share_img);
		$this->assign('share_data', $this->get_wechat_share_content($share_data));
		$this->assign('page_title', $share_title);
		$this->assign('description', $share_desc);
		$this->assign('keywords', C('shop.shop_keywords'));
		$this->assign('is_app',$GLOBALS['MALL_CLIENT']);//判断是否为app端
		$this->display();
	}
	...
	{
		$number = input('post.number', 10);//每页多少个商品
		$cat_id_yuan = dao('category')->where(array('cat_name'=>'版画'))->getField('cat_id');
		$cat_id =input('post.cat_id',0);
		if(empty($cat_id)){
			$cat_id=$cat_id_yuan; 
		}
		$condition = array('intro' => input('post.type', ''),'cat_id'=> $cat_id,'page'=>input('post.page',1));//默认给版画id
		...
		$this->response(array('error' => 0, 'data' => $res, 'endtime' => date('Y-m-d H:i:s', $endtime),'totalPage'=>$this->totalPage));
	}
	...
	private function getGoodsList($param = array(), $size = 10)
	{
		...
		if ($list === false) {
			...
			if ($res) {
				...
				$this->totalPage=empty($data['totalPage']) ? 1 : $data['totalPage'];
				//S($cache_id, $list, 600);
			}
		}
