...
//		$this->assign('page_title', $position['title']);
		...
		/*$goods_desc = preg_replace("/<img\s+src=\"([^\"]+)\"\s*\/?>/is", "<img data-src=\"\\1\" class=\"swiper-lazy\"><div class=\"swiper-lazy-preloader\"></div>", $goods_desc);
	...
	public function actionSharing()
	{
		if (IS_AJAX) {
			...
			$qrcode = $file . 'goods_qrcode_' . $share_type . $errorCorrectionLevel . $matrixPointSize . '_' . $this->goods_id . '_' . $user_id . '.png';
			$outImg = $file . 'goods_share_' . $share_type . $errorCorrectionLevel . $matrixPointSize . '_' . $this->goods_id . '_' . $user_id . '.png';

			if (!file_exists($outImg)) {
				if ($goods_url) {
					\App\Extensions\QRcode::png($goods_url, $qrcode, $errorCorrectionLevel, $matrixPointSize, 2);
					$img = new \Think\Image();
					$bg_width = $img->open($bgImg)->width();
					$bg_height = $img->open($bgImg)->height();
					$goods_thumb = $goods_thumb_file . 'goods_thumb_' . $this->goods_id . '.png';
					$goods['goods_img'] = !empty($goods['goods_img']) ? $goods['goods_img'] : $goods['goods_thumb'];
					$goods_title = strip_tags(html_out($goods['goods_name']));
					$goods_title_first = sub_str($goods_title, 24, true, 0);
					...
					$goods_description = $goods_title_first . '\\r\\n' . $goods_price;
					$goods_description = str_replace(array('\\r\\n', '\\n', '\\r'), PHP_EOL, htmlspecialchars($goods_description));

					if (!file_exists($goods_thumb)) {
						if (C('shop.open_oss') == 1) {
							$goodsimg = \App\Extensions\Http::doGet($goods['goods_img']);
							$goodsthumb = $goods_thumb;
							file_put_contents($goodsthumb, $goodsimg);
						}
						else {
							$goodsthumb = str_replace(__HOST__, dirname(ROOT_PATH), $goods['goods_img']);
						}

						$img->open($goodsthumb)->thumb($bg_width, $bg_width)->save($goods_thumb);
					}

					$img->open($bgImg)->water($goods_thumb, array(0, 0), 100)->text($goods_description, dirname(ROOT_PATH) . '/data/attached/fonts/msyh.ttf', 18, '#0e0e0e', array(20, $bg_width + 20))->save($outImg);

					if (!file_exists($avatar)) {
						$headimg = \App\Extensions\Http::doGet($users['user_picture']);
						$avatar_open = $avatar;
						file_put_contents($avatar_open, $headimg);
						if ($headimg === false || strtolower(substr($users['user_picture'], 0, 4)) !== 'http') {
							$avatar_open = dirname(ROOT_PATH) . $users['user_picture'];
						}

						$img->open($avatar_open)->thumb(80, 80)->save($avatar);
					}

					$img->open($qrcode)->water($avatar, 5, 100)->save($qrcode);
					$nickname = $users['nick_name'];
					$qr_left = 20;
					$qr_top = $bg_width + 120;
					$logo_width = $img->open($qrcode)->width();
					$text_left = $logo_width + 80;
					$text_top = $qr_top;

					if ($share_type == 1) {
						$text_description = '我是' . $nickname . '\\r\\n' . '我为' . C('shop.shop_name') . '代言';
					}
					else {
						$text_description = '我是' . $nickname . '\\r\\n' . '推荐好友得好礼';
					}
					...
					$img->open($outImg)->water($qrcode, array($qr_left, $qr_top), 100)->text($text_description, dirname(ROOT_PATH) . '/data/attached/fonts/msyh.ttf', 16, '#0e0e0e', array($text_left, $text_top))->save($outImg);
				}
