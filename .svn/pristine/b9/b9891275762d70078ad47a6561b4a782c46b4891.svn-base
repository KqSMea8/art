...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function __construct()
    {
        ...
        $files = array('order', 'clips', 'transaction','function');
    ...
    // 发放或者更新优惠券
    private function getArtzheCounpons()
    {
        $uid = $_SESSION['user_id'];

        // -- 用户的优惠券列表
        $endTime = $this->artConfig['queen_endtime'];
        $where = 'where cu.user_id =  ' . $uid . ' and c.cou_end_time = ' . $endTime . ' and c.review_status = 3';
        $sql = 'select c.cou_id,c.cou_goods,cu.is_use from ' . $GLOBALS['ecs']->table('coupons_user') . ' as cu left join ' . $GLOBALS['ecs']->table('coupons') . ' as c on c.cou_id = cu.cou_id ' . $where;
        $counps = $GLOBALS['db']->getAll($sql);
        $goodsCoupon = array();
        foreach ($counps as $k => $v) {
            $goodsCoupon[$v['cou_goods']]['cou_id'] = $v['cou_id'];
            $goodsCoupon[$v['cou_goods']]['is_use'] = $v['is_use'];
        }

        // -- 从接口获取数据，网络请求
        $url = $this->artConfig['queen_artzheurl']. "getUserBargains";
        $postStr = des_encode(json_encode(array('uid' => $uid, 'user_ip' => $_SERVER['REMOTE_ADDR'])));
        $data = curl_request($url,array('param'=>$postStr));
        Log::write('getArtzheCounpons data:' . $data);
        ...
        if ($data['message'] == 'success') {
            ...
            foreach ($array as $k => $v) {
                ...
                if (array_key_exists($v['goods_id'], $goodsCoupon)) {
                    if ($goodsCoupon[$v['goods_id']]['is_use'] == 0) {
                        $updateSql = 'UPDATE ' . " {pre}coupons " . ' SET cou_money = ' . $money . ' , cou_man = ' . $money . ' where cou_id = ' . $goodsCoupon[$v['goods_id']]['cou_id'];
                        $this->db->query($updateSql);
                    }
                } else {
                    $this->createArtzheQueenCoupon($money, $uid,$v['goods_id'],$endTime);
                }
            }
        } else {
            Log::write('getArtzheCounpons error:' . $data['message']);
        }
    }

    private function createArtzheQueenCoupon($money,$uid,$goods_id,$endTime)
    {
        $coun_name = '女王节优惠券'.substr(strval(time()),2,9).rand(100,999);
        $cou_total = 1;
        $cou_man = $money;
        $cou_money = $money;
        $cou_man = $cou_money;
        $cou_user_num = 1;//每个人限量1张
        $cou_goods = "$goods_id";//制定的商品 使用数组隔开
        $spec_cat = '';//分类id  逗号隔开
        $cou_start_time = time() - 3600 * 24;
        $cou_end_time = $endTime;
        $cou_type = 4;// 会员增劵
        $cou_get_man = 0;
        $cou_ok_user = '0';
        $cou_ok_goods = '0';
        $cou_ok_cat = "";
        $cou_add_time = $cou_start_time;
        $adminru = '';
        $cou_intro = '下单立减';
        $cou_title= '女王节优惠券';

        $insterSqlcou = 'INSERT INTO ' . " {pre}coupons " . (" ( cou_name,cou_total,cou_man,cou_money,cou_user_num,cou_goods,spec_cat,cou_start_time,cou_end_time,cou_type,cou_get_man,cou_ok_user,cou_ok_goods,cou_ok_cat,cou_intro,cou_add_time,ru_id,cou_title,review_status,review_content) VALUES ( '" . $coun_name . "',\r\n            '" . $cou_total . "',\r\n            '" . $cou_man . "',\r\n            '" . $cou_money . "',\r\n            '" . $cou_user_num . "',\r\n            '" . $cou_goods . "',\r\n            '" . $spec_cat . "',  \r\n            '" . $cou_start_time . "',\r\n            '" . $cou_end_time . "',\r\n            '" . $cou_type . "',\r\n            '" . $cou_get_man . "',\r\n            '" . $cou_ok_user . "',\r\n            '" . $cou_ok_goods . "',\r\n            '" . $cou_ok_cat . "',\r\n            '" . $cou_intro . "',\r\n            '" . $cou_add_time . "',\r\n            '") . $adminru['ru_id'] . ("',\r\n            '" . $cou_title . "',\r\n            '3',\r\n            ''    \r\n            )");
        $re = $this->db->query($insterSqlcou);

        // 给用户插入优惠券
        if ($re) {
            $time = gmtime();
            $uc_sn = $time . rand(10, 99);
            $insterSqluser = 'INSERT INTO ' . " {pre}coupons_user " . (' (`user_id`,`cou_id`,`uc_sn`) VALUES (' . $uid . ',' . $this->db->getLastInsID() . ',\'' . $uc_sn . '\') ');
            $this->db->query($insterSqluser);
        }
    }
    ...
    {
        // 添加条件，女王节优惠活动修改购物券的金额或者发放购物券
        if ($this->artConfig['queen_online']) {
            $this->getArtzheCounpons();
        }
        ...
        //获取订单的商品类型
        if(!empty($cart_value)){
            $cart_value = strval($cart_value);
            $catIds= dao('cart')
                ->field('dsc_goods.cat_id')
                ->join('JOIN dsc_goods ON dsc_goods.goods_id = dsc_cart.goods_id')
                ->where(['dsc_cart.rec_id' => ['in',$cart_value]])
                ->select();
        ...
        if (isset($payment_list)) {
            foreach ($payment_list as $key => $payment) {
                if ($payment['pay_code'] != 'wxpay' && $payment['pay_code'] != 'llpay') {
                    unset($payment_list[$key]);
                }


                if ($payment['pay_code'] == 'wxpay' && in_array('1488',$catIds)) {//如果有原作，不显示微信支付
                    unset($payment_list[$key]);
                }

                if($payment['pay_code'] == 'wxpay'){// 默认选择支付方式 微信支付
                    /*$payment_selected = $payment;//payment_info($payment['pay_id']);
    ...
    public function actionBuy()
    {
        ...
        unset($_SESSION['flow_order']['uc_id']);
        unset($_SESSION['flow_order']['vc_id']);
        $flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : GENERAL_GOODS;
      ...
      /*  $sql = 'SELECT COUNT(*) FROM {pre}cart  WHERE ' . $this->sess_id . ('AND parent_id = 0 AND is_gift = 0 AND rec_type = \'' . $flow_type . '\'');
        ...
        $user_address = get_order_user_address_list($_SESSION['user_id']);
        if ($direct_shopping != 1 && !empty($_SESSION['user_id'])) {
            $_SESSION['browse_trace'] = url('goods/index/index');
        } else {
            $_SESSION['browse_trace'] = url('flow/index/index');
        }
        ...
        //TODO 获取商品信息
       // $cart_goods_list = cart_goods($flow_type, $cart_value, 1, $this->region_id, $this->area_id, $consignee, $store_id);
        $goods_info =dao('goods')->where('goods_id = '.$goods_id.' AND is_delete =0 AND is_on_sale = 1 ')->find();
        if (empty($goods_info)) {
            $this->redirect('/');
        }
        $store_goods_id = '';
        if ($goods_info) {
            $amount = 0;
            $goods_price_amount = 0;
            $amount += $goods_info['shipping_fee'];
            $amount += $goods_info['shop_price'];
            $goods_price_amount += $goods_info['shop_price'];
            ...
            $goods_info['amount'] = $amount ? $amount : 0;
            $goods_info['goods_price_amount'] = $goods_price_amount ? price_format($goods_price_amount, false) : 0;
            $goods_info['formated_goods_price'] = price_format($goods_info['shop_price'], false);
            $goods_info['formated_subtotal'] = price_format($amount, false);
            $goods_info['goods_number'] = 1;
            $goods_info['goods_thumb'] = get_image_path($goods_info['goods_thumb']);
            $goods_info['formated_market_price'] = price_format($goods_info['market_price'], false);
            $goods_info['url'] = build_uri('goods', array('gid' => $goods_info['goods_id']), $goods_info['goods_name']);
            $goods_info['attr_number'] = 1;
       ...
       /* if (0 < $store_id) {
        ...
        $ru_goods_list['goods_list'][] = $goods_info;
        $ru_goods_list['ru_name'] = get_shop_name($goods_info['user_id'], 1);
        $ru_goods_list['ru_id'] = $goods_info['user_id'];
        $ru_goods_list['goods_count'] = 1;
        $ru_goods_list['amount'] = $goods_info['amount'];
        //配送信息
        $ru_shippng = get_ru_shippng_info_buy($goods_info, $consignee);
        $ru_goods_list['shipping'] = $ru_shippng['shipping_list'];
        $ru_goods_list['is_freight'] = $ru_shippng['is_freight'];

        if (!empty($ru_goods_list['shipping'])) {
            $ru_goods_list['tmp_shipping_id'] = isset($ru_goods_list['shipping'][0]['shipping_id']) ? $ru_goods_list['shipping'][0]['shipping_id'] : 0;

            foreach ($ru_goods_list['shipping'] as $kk => $vv) {
                if ($vv['default'] == 1) {
                    $ru_goods_list['tmp_shipping_id'] = $vv['shipping_id'];
                    continue;
                }
            }
        }
        $goods_list[] = $ru_goods_list;
      ...
      /*  if ($flow_type != CART_EXCHANGE_GOODS && $flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_GROUP_BUY_GOODS) {
       ...
       // $cart_goods = get_new_group_cart_goods($cart_goods_list);
       // $total = order_fee($order, $goods_list, $consignee, 0, '', 0, $cart_goods_list, 0, 0, $store_id, $store_seller);
        $total = array('real_goods_count' => 1, 'gift_amount' => 0, 'goods_price' => $goods_info['shop_price'], 'market_price' => $goods_info['market_price'], 'discount' => 0, 'pack_fee' => 0, 'card_fee' => 0, 'shipping_fee' => $goods_info['shipping_fee'], 'shipping_insure' => 0, 'integral_money' => 0, 'bonus' => 0, 'coupons' => 0, 'value_card' => 0, 'surplus' => 0, 'cod_fee' => 0, 'pay_fee' => 0, 'tax' => 0, 'presale_price' => 0);
        $total['exchange_integral'] = 0;
        $total['formated_goods_price'] = price_format($total['goods_price'], false);
        $total['formated_market_price'] = price_format($total['market_price'], false);
        $total['amount'] = $goods_info['amount'];
        ...
        //获取订单的商品类型
        if(!empty($goods_id)){
            $catIds= dao('goods')->field('cat_id')->where(['goods_id' => $goods_id])->find();
            $catIds[] = $catIds['cat_id'];
        }else{
            $catIds = [];
        }
        if (isset($payment_list)) {
            foreach ($payment_list as $key => $payment) {
                ...
                if($payment['pay_code'] == 'llpay'){// 默认选择支付方式 连连支付
                    /*$payment_selected = $payment;//payment_info($payment['pay_id']);
                    if (file_exists(ADDONS_PATH . 'payment/' . $payment_selected['pay_code'] . '.php')) {
                        $payment_selected['format_pay_fee'] = strpos($payment_selected['pay_fee'], '%') !== false ? $payment_selected['pay_fee'] : price_format($payment_selected['pay_fee'], false);
                        $this->assign('payment_selected', $payment_selected);
        ...
        $use_integral = C('shop.use_integral');
        if ((!isset($use_integral) || $use_integral == '1') && 0 < $_SESSION['user_id'] && 0 < $user_info['pay_points'] && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS)) {
            $order_max_integral = integral_of_value($goods_info['integral']);
            $this->assign('allow_use_integral', 1);
            $this->assign('order_max_integral', $order_max_integral);
            $this->assign('your_integral', $user_info['pay_points']);
            $integral_scale = C('shop.integral_scale');
    ...
    public function actionDonebuy()
    {
        ...
        if (!isset($_SESSION['goods_id'])) {
            //show_message(L('no_goods_in_cart'), '', url('goods/index/index'), 'warning');
            $this->redirect('user/order/index');
        }
        ...
        //TODO
        if (C('shop.use_storage') == '1' && C('shop.stock_dec_time') == SDT_PLACE) {
            $cart_goods_stock = get_cart_goods($_SESSION['goods_id']);
            $_cart_goods_stock = array();
        ...
        if (0 < $user_id) {
           ...
           // $flow_points = flow_available_points($_SESSION['cart_value']);
            $flow_points = integral_of_value($goods_info['integral']);
            $user_points = $user_info['pay_points'];
            $order['integral'] = min($order['integral'], $user_points, $flow_points);
      ...
      /*  foreach ($cart_goods_list as $key => $val) {
       ...
       // $total = order_fee($order, $cart_goods, $consignee, 1, $_SESSION['cart_value'], $pay_type, $cart_goods_list, 0, 0, $store_id);
        ...
        $order['bonus'] = $total['bonus'];
        $order['coupons'] = $total['coupons'];
        $order['use_value_card'] = empty($total['use_value_card'])?0:$total['use_value_card'];
        $order['goods_amount'] = $total['goods_price'];
        $order['cost_amount'] = $total['cost_price'] ? $total['cost_price'] : 0;
        $order['discount'] = $total['discount'] ? $total['discount'] : 0;
        $order['surplus'] = $total['surplus'];
        ...
        $order['integral_money'] = empty($total['integral_money'])?0:$total['integral_money'];
        $order['integral'] = empty($total['integral'])?0:$total['integral'];
        $integral_scale = C('shop.integral_scale');
        ...
        if (is_dir(APP_DRP_PATH) && $order['extension_code'] != 'bargain_buy') {
           ...
           // $sql = 'INSERT INTO ' . $this->ecs->table('order_goods') . '( ' . 'order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, goods_price, commission_rate, ' . 'goods_attr, is_real, extension_code, parent_id, is_gift,freight, tid,shipping_fee, model_attr, goods_attr_id, ru_id, shopping_fee, warehouse_id, area_id, is_distribution, drp_money) ' . (' SELECT \'' . $new_order_id . '\', ca.goods_id, ca.goods_name, ca.goods_sn, ca.product_id, ca.goods_number, ca.market_price, ca.goods_price, ca.commission_rate, ca.goods_attr, ') . 'ca.is_real, ca.extension_code, ca.parent_id, ca.is_gift,ca.freight, ca.tid,ca.shipping_fee,ca.model_attr, ca.goods_attr_id, ca.ru_id, ca.shopping_fee, ca.warehouse_id,ca.area_id,' . ('g.is_distribution*\'' . $is_distribution . '\' as is_distribution, ') . ('g.dis_commission*g.is_distribution*ca.goods_price*ca.goods_number/100*\'' . $is_distribution . '\' as drp_money') . ' FROM ' . $this->ecs->table('cart') . ' ca' . ' LEFT JOIN  {pre}goods as g ON ca.goods_id=g.goods_id' . ' WHERE ca.' . $this->sess_id . (' AND is_checked=1  AND ca.rec_type = \'' . $flow_type . '\'') . $goodsIn;
            $sql = 'INSERT INTO ' . $this->ecs->table('order_goods') . '( ' .
                ...
                (' SELECT \'' . $new_order_id . '\', g.goods_id, g.goods_name, g.goods_sn, g.product_id, \'1\', g.market_price, g.shop_price, g.commission_rate, \'\', ') .
                'g.is_real, g.extension_code, \'0\', \'0\',g.freight, g.tid,g.shipping_fee,g.model_attr, \'\', g.user_id, \'0\', \'0\',\'0\',' .
                ('g.is_distribution*\'' . $is_distribution . '\' as is_distribution, ') . ('g.dis_commission*g.is_distribution*g.shop_price/100*\'' . $is_distribution . '\' as drp_money') .
                ' FROM ' . $this->ecs->table('goods') . ' g'  .
           ...
           /* $goodsIn = '';
            ...
            $sql = 'INSERT INTO ' . $this->ecs->table('order_goods') . '( ' .
                ...
                (' SELECT \'' . $new_order_id . '\', goods_id, goods_name, goods_sn, product_id, \'1\', market_price, commission_rate, ') .
                'shop_price, \'\', is_real, extension_code, \'0\', \'0\', model_attr, \'\', user_id, \'0\', \'0\', \'0\', freight, tid, shipping_fee' .
                ' FROM ' . $this->ecs->table('goods') . ' WHERE  goods_id = \'' . $goods_id . '\'';
        ...
        if (1 <= $goods_info['user_id']) {
            $sql = 'SELECT seller_email FROM ' . $GLOBALS['ecs']->table('seller_shopinfo') . ' WHERE ru_id = \'' .$goods_info['user_id'] . '\'';
            $service_email = $GLOBALS['db']->getOne($sql);
        } else {
            $service_email = C('shop.service_email');
        }
        ...
        if ($order['order_amount'] <= 0) {
           /* $sql = 'SELECT goods_id, goods_name, goods_number AS num FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE is_real = 0 AND extension_code = \'virtual_card\'' . ' AND ' . $this->sess_id . (' AND rec_type = \'' . $flow_type . '\'');
            ...
            $virtual_goods['virtual_card'][] = array('goods_id' => $goods_info['goods_id'], 'goods_name' => $goods_info['goods_name'], 'num' => 1);
        ...
        if (0 < $order['order_amount'] && $order['order_amount']<= 5000) {
            include_once ADDONS_PATH . 'payment/' . $payment['pay_code'] . '.php';
            $pay_obj = new $payment['pay_code']();
            $pay_online = $pay_obj->get_code($order, unserialize_config($payment['pay_config']));
            $order['pay_desc'] = $payment['pay_desc'];
        ...
        if ($order['order_amount']<= 5000) {
            S('order_hash_' . $_SESSION['user_id'], null);
            $this->assign('page_title', L('order_success'));
            $this->display('done');
        } else{
            //大于5000，发短信通知工作人员
            $code = $order['order_sn'].','.$order['order_amount'];
            $this->sendInformation($this->artConfig['mobile1'],$code);
            $this->sendInformation($this->artConfig['mobile2'],$code);
            $this->sendInformation($this->artConfig['mobile3'],$code);
            //跳到订单页面
            $this->redirect('user/order/index');
        }
    ...
    public function sendInformation($mobile = '',$code=''){
        $url=$this->artConfig['ARTZHE_URL']."ArtworkGoods/sendInformation";
        $postStr=array(
            'mobile'=>$mobile,
            'code'=>$code,
        );
        //$post_data=array('param'=>des_encode(json_encode($postStr)));
        $a = $this->curl_request($url,$postStr);
    ...
    public function actionSetAddress()
    {
        if (IS_AJAX) {
            ...
            if (!empty($address)) {
                ...
                if(isset($_SESSION['goods_id'])){
                    echo json_encode(array('url' => url('flow/index/buy',array('goods_id'=>$_SESSION['goods_id'])), 'status' => 1));
                }else{
