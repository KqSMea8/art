...
class OrderController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	public function actionDetail()
	{
		...
		//---
		$goods_satus=1;//1--订单中的商品库存正常,   0--订单中的商品库存缺少,且订单未支付
		//---
		foreach ($goods_list as $key => $value) {
			//---
			//如果减库存时间是付款时，判断订单中的商品是否有库存
			if($GLOBALS['_CFG']['stock_dec_time'] == 2){
				if($value['goods_stock_number']<=0 && $order['pay_status'] == PS_UNPAYED){//没有库存且订单未支付
					$goods_satus=0;
				}
			}
			//---
		...
		if ((0 < $order['order_amount']) && (($order['pay_status'] == PS_UNPAYED) || ($order['pay_status'] == PS_PAYED_PART)) && ($order['shipping_status'] == SS_UNSHIPPED)) {
			$payment_list = available_payment_list(false, 0, true); //获取支付方式列表

			//获取订单的商品类型
			$goodsIds = array_column($goods_list,'goods_id');
			if(!empty($goodsIds)){
				$goodsIds = implode(',',$goodsIds);
				$catIds= dao('goods')->field('cat_id')->where(['goods_id' => ['in',$goodsIds]])->select();
			...
			if (is_array($payment_list)) {
				foreach ($payment_list as $key => $payment) {
					...
					if ($payment['pay_code'] == 'wxpay') {
						...
						if (in_array('1488',$catIds)) {//如果有原作，不显示微信支付
							unset($payment_list[$key]);
						}
		...
		//---
		$this->assign('goods_satus', $goods_satus);//1--订单中的商品库存正常   0--订单中的商品库存缺少
		//---
