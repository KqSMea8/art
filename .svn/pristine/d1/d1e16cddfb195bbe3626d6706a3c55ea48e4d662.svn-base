...
class UserLogic extends BaseLogic
{
    ...
    {
        $list = $this->model->where(['id'=>['in'=>$idList], 'is_deleted'=>'N', 'is_banned'=>'N'])->select();
        if (empty($list))
        {
            return [];
        } else {
    ...
    public function getUserDesc($uid,$loginUid = ''){
        $userinfo = $this->field('id,nickname,name,gender,face,motto,category')->where(['id' => $uid, 'is_deleted' => 'N', 'banned_to' => '-1'])->find();
        $artModel = new ArtworkModel();
        $userFollowerModel = new UserFollowerModel();
        if($uid > 0 && $loginUid == $uid){
            $artTotal = $artModel->where(['artist' => $uid, 'is_deleted' => 'N', 'update_times' => ['gt', 0]])->count();
        }else{
            $artTotal = $artModel->where(['artist' => $uid, 'is_deleted' => 'N', 'state' => '1' , 'update_times' => ['gt', 0]])->count();
        }
