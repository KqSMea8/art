...
function get_goods_sale($order_id = 0, $order = array())
{
	...
	if ($is_volume == 1) {
		...
		if ($is_update_sale < 1) {
			...
			$where = $where.' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0 ';
