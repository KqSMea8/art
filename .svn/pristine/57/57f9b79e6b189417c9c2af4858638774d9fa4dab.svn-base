...
use Api\Model\CommentLikeModel;
...
class CommentLogic extends BaseLogic
{
  ...
  public function getAuthorByIdAndType($id,$type){
    ...
    public function getLastFace($topic_id,$type = 1){
      $userLogic = new UserLogic();
      $return = []; //最多显示12个留言者头像
      $list = $this->distinct(true)->field('commenter,create_time')->where(['is_deleted' => 'N','topic_id' => $topic_id,'type' => $type,'parent_id'=>0])->order('create_time desc')->limit(12)->select();

      foreach ($list as $key => $value) {
        $face = $userLogic->where(['id' => $value['commenter']])->getField('face');
        $return[] = Util::getImgUrlById($face);
      }
      return $return;
    }
