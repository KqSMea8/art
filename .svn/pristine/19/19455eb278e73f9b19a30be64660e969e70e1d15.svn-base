...
if ($_REQUEST['act'] == 'advanced_search') {
	...
	if ($search_type == 0) {
		...
		$sql = 'SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' AS g ' . $leftJoin . ('WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 ' . $attr_in . ' ') . 'AND (( 1 ' . $categories . $keywords . $where_price_min . $where_price_max . $intro . $outstock . ' ) ' . $tag_where . ' ) ' . $act_name . (' ' . $cou_goods_where . ' ');
		$count = $db->getOne($sql);
