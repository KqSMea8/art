...
if ($action == 'register') {
	...
	if ($action == 'order_list' || $action == 'order_recycle') {
		...
		if ($action == 'order_detail' || $action == 'auction_order_detail') {
			...
			$orders = get_user_orders($user_id, $record_count, $page, $show_type, $where, $order, $show_type, $pagesize = 10);
