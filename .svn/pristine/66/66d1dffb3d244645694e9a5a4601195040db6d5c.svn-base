...
function get_category_store_list($ru_id, $is_url = 0, $level = 0, $parent_id = 0)
{
	...
	$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('merchants_category') . (' WHERE ' . $where . ' AND parent_id = \'' . $parent_id . '\' ORDER BY sort_order, cat_id DESC');
	$filter['record_count'] = $GLOBALS['db']->getOne($sql);
	$filter = page_and_size($filter);
	$sql = 'SELECT cat_id, cat_name, parent_id, user_id AS ru_id, sort_order, ' . 'measure_unit, grade, is_show, show_in_nav FROM ' . $GLOBALS['ecs']->table('merchants_category') . (' WHERE ' . $where . ' AND parent_id = \'' . $parent_id . '\' ORDER BY cat_id , sort_order ') . ' LIMIT ' . $filter['start'] . (',' . $filter['page_size']);
	$res = $GLOBALS['db']->getAll($sql);
...
function shipping_info($shipping, $select = array())
{
	...
	$sql = 'SELECT ' . $select . ' FROM ' . $GLOBALS['ecs']->table('shipping') . (' WHERE ' . $where . ' AND enabled = 1 LIMIT 1');
	$row = $GLOBALS['db']->getRow($sql);
