...
function order_list($page = 0)
{
	...
	if ($result === false) {
		...
		case CS_NEW_ORDER:
			$where .= ' AND o.add_time >= ' . $firstSecToday . ' AND o.add_time <= ' . $lastSecToday;
			break;

		case CS_NEW_PAID_ORDER:
			$where .= ' AND o.pay_time >= ' . $firstSecToday . ' AND o.pay_time <= ' . $lastSecToday;
			break;
...
function download_orderlist($result)
{
	...
	for ($i = 0; $i < $count; $i++) {
		...
		$order_count = i($result[$i]['total_fee']) + $tax + $shipping_fee + $insure_fee + $pay_fee + i($result[$i]['pack_fee']) + i($result[$i]['card_fee']) - $discount;
		$data .= $order_sn . ',' . $seller_name . ',' . $order_user . ',' . $order_time . ',' . $consignee . ',' . $tel . ',' . $address . ',' . i($result[$i]['total_fee']) . ',' . $order_count . ',' . $shipping_fee . ',' . $insure_fee . ',' . $pay_fee . ',' . $surplus . ',' . $integral_money . ',' . $bonus . ',' . $tax . ',' . $discount . ',' . $coupons . ',' . $order_amount . ',' . $order_status . ',' . $pay_status . ',' . $shipping_status . ',' . $froms . ',' . $pay_name . "\n";
	}
